...
cs2_get_inventory/
├── src/
│   ├── index.js          # entry point: Steam login, GC connection, saving results
│   ├── items.js          # inventory processing (names, floats, stickers, caskets)
│   ├── prices.js         # CSFloat price fetching (cache, rate limit, retries)
...
- Node.js
- Packages: `steam-user`, `globaloffensive`, `steam-totp`, `@node-steam/vdf`, `csgo-fade-percentage-calculator`
...
4. Fetch the lowest buy-now price from the CSFloat market for every marketable item (`price_csfloat_usd`, in USD). Doppler prices are filtered by phase via `paint_index`.
